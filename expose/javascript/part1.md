...
5. The code returns an error because the variable `result` is declared within the `if` code-block. Therefore, `result` is only visible within that `if` code-block, and the `console.log` at line 13 is outside the code block, and would not see `result`, which causes the error.
